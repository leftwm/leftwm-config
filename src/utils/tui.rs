...
use crate::config::filehandler::{load, save_to_file};
...
use crate::utils::tui::Window::WindowRules;
                    ...
                    Window::Home => {
                        if let Some(s) = self.current_popup {
                            match s {
                                ...
                                2 => {
                                    popups::max_window_width(&mut self.current_popup_state, f);
                                }
                ...
                match key.code {
                    ...
                    KeyCode::Down => {
                        if self.current_popup.is_none() {
                            match self.config_list_state.selected() {
                                None => self.config_list_state.select(Some(0)),
                                Some(_) => {
                                    next(&mut self.config_list_state, self.config_list.len())
                                }
                            }
                        } else if let Some(s) = self.current_popup {
                            match self.current_window {
                    ...
                    KeyCode::Right => {
                        if let Window::Workspaces { index, empty } = self.current_window {
                            if !empty {
                                if index
                                    >= self.current_config.workspaces.as_ref().unwrap().len() - 1
                                {
                            ...
                            if !empty {
                                if index >= self.current_config.tags.as_ref().unwrap().len() - 1 {
                        ...
                        } else if let Window::WindowRules { index, empty } = self.current_window {
                            if self.current_popup.is_some_and(|i| *i == 2) {
                                if let PopupState::Int { current, min, max } =
                                    self.current_popup_state
                                {
                                    if current < max {
                                        self.current_popup_state = PopupState::Int {
                                            current: current + 1,
                                            min,
                                            max,
                                        }
                                ...
                                if index
                                    >= self.current_config.window_rules.as_ref().unwrap().len() - 1
                                {
                                    self.current_window = Window::WindowRules { index: 0, empty }
                                } else {
                                    self.current_window = Window::WindowRules {
                                        index: index + 1,
                                        empty,
                                    }
                                }
                    ...
                    KeyCode::Left => {
                        if let Window::Workspaces { index, empty } = self.current_window {
                            ...
                            if !empty {
                                if index == 0 {
                                    self.current_window = Window::Tags {
                                        index: (self.current_config.tags.as_ref().unwrap().len()
                                            - 1),
                                        empty,
                                    };
                                } else {
                    ...
                    KeyCode::Enter => {
                        match self.current_window {
                                            ...
                                            9 => {
                                                ...
                                                for l in &self.current_config.layouts {
                                                    match l {
                                                        ...
                                                        WMLayout::GridHorizontal => {
                                                            selected.push(3)
                                                        }
                                                        WMLayout::EvenHorizontal => {
                                                            selected.push(4)
                                                        }
                                            ...
                                            1 => {
                                                    ...
                                                    &self.current_popup_state
                                                {
                                                    if let Some(s) = s.selected() {
                                                        match s {
                                                            0 => {
                                                                self.current_config.mousekey = None
                                                            }
                                                            1 => {
                                                    ...
                                                    &self.current_popup_state
                                                {
                                                    ...
                                                    for s in &l.selected {
                                                        match s {
                                                            ...
                                                            2 => {
                                                                layouts.push(WMLayout::MainAndDeck)
                                                            }
                                                            3 => layouts
                                                                .push(WMLayout::GridHorizontal),
                                                            4 => layouts
                                                                .push(WMLayout::EvenHorizontal),
                                                            5 => {
                                                                layouts.push(WMLayout::EvenVertical)
                                                            }
                            ...
                            Window::Tags { index, empty } => match self.current_popup {
                                Some(_) => {
                                    *self
                                        ...
                                        .unwrap() = {
                                        if let PopupState::String(s) =
                                            self.current_popup_state.clone()
                                        {
                                            s
                                        } else {
                                            panic!("Invlaid popup state");
                                        }
                                    };
                                    self.current_popup = None;
                            ...
                            Window::WindowRules { index, empty } => match self.current_popup {
                                Some(0) => {
                                    self.current_config
                                        .window_rules
                                        .as_mut()
                                ...
                                Some(_) => {}
                                None => match self.config_list_state.selected() {
                                    Some(2) => {
                                        if empty {
                                            if self
                                                .current_config
                                            ...
                                            self.current_popup = Some(0);
                                            self.current_popup_state = PopupState::String(
                                                self.current_config
                                                    .window_rules
                                                    .as_ref()
                                                    .unwrap()
                                    ...
                                    Some(4) => {
                                        ...
                                        self.current_popup_state = PopupState::Int {
                                            current: self
                                                ...
                                                as isize,
                                            min: 1,
                                            max: self.current_config.tags.as_ref().unwrap().len()
                                                as isize,
                                        }
                                    }
                                    Some(5) => {
                                    ...
                                    Some(8) => {
                                        ...
                                        if index
                                            ...
                                            && !self
                                                ...
                                                .is_empty()
                                        {
                                            self.current_window = WindowRules {
                                                index: index - 1,
                                                empty,
                                            };
                                        } else if self
                                ...
                                if let PopupState::MultiList(l) = &mut self.current_popup_state {
                                    if l.selected.contains(&l.liststate.selected().unwrap_or(14)) {
                                        let index = l
                                            ...
                                            .iter()
                                            .position(|x| {
                                                *x == l.liststate.selected().unwrap_or(14)
                                            })
                                            .unwrap();
                    ...
                    KeyCode::Backspace => {
                        match self.current_window {
                            ...
                            Window::WindowRules { .. } => match self.current_popup {
                                ...
                                Some(2) => {}
                                Some(3) => {}
                                None => self.current_window = Window::Home,
                                _ => {}
                            },
                            _ => {}
                        }
                    }
                    KeyCode::Delete => match self.current_window {
                    ...
                    .is_some_and(|v| !v.is_empty())
                {
                    let mut vec = vec![
                        ListItem::new(format!(
                            "{} out of {}",
                            index + 1,
                            self.current_config.window_rules.as_ref().unwrap().len()
                        )),
                        ListItem::new("--------------------------"),
                        ListItem::new(format!(
                            "Title - {:?}",
